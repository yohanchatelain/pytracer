"""
Comprehensive tests for the improved pytracer architecture.

This module tests the new configuration system, error handling,
and wrapper system to ensure they work correctly and are properly isolated.
"""
...
class TestConfigurationSystem(unittest.TestCase):
    ...
    def test_tracer_config_validation(self):
        """Test configuration validation."""
        # Valid configuration should not raise
        config = TracerConfig.from_dict(self.test_config_data)
        config.validate()  # Should not raise
        
        # Invalid file path should raise
        invalid_data = self.test_config_data.copy()
        invalid_data["include_file"] = ["/nonexistent/path.txt"]
        
        config = TracerConfig.from_dict(invalid_data)
    ...
    def test_configuration_manager_from_environment(self):
        """Test ConfigurationManager creation from environment."""
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f:
            json.dump(self.test_config_data, f)
            config_path = f.name
        
        try:
            with patch.dict(os.environ, {'PYTRACER_CONFIG': config_path}):
                manager = ConfigurationManager.from_environment()
                config = manager.get_config()
                self.assertEqual(config.modules_to_load, ["numpy", "scipy"])
        finally:
            os.unlink(config_path)
    
    def test_configuration_manager_no_environment(self):
        """Test error when PYTRACER_CONFIG not set."""
        with patch.dict(os.environ, {}, clear=True):
            with self.assertRaises(ConfigurationError):
                ConfigurationManager.from_environment()
...
class TestWrapperSystem(unittest.TestCase):
    """Test the improved wrapper system."""
...
class TestWrapperContext(unittest.TestCase):
    """Test WrapperContext functionality."""
...
class TestIntegration(unittest.TestCase):
    """Integration tests for the new architecture components."""
    
    def test_config_and_wrapper_integration(self):
        """Test integration between configuration and wrapper systems."""
        ...
        def tracer_callback(event, context):
            # Mock tracer that uses configuration
            if manager.get_config().logger_config.level == LogLevel.DEBUG:
                print(f"Tracing {event}: {context}")
        
        engine = create_default_wrapper_engine(tracer_callback)
        
        # Test wrapping with configuration context
        def test_func():
            return 42
        ...
        context = WrapperContext(
            object_name="test_func",
            object_type=ObjectType.FUNCTION,
            module_name=config.modules_to_load[0] if config.modules_to_load else "unknown"
        )
        
        result = engine.wrap_object(test_func, context)
        self.assertTrue(result.is_success())
        
        wrapped_func = result.unwrap()
        # This should trigger the tracer callback
        self.assertEqual(wrapped_func(), 42)
    
    def test_error_handling_integration(self):
        """Test error handling across components."""
        collector = ErrorCollector()
        
        # Test configuration error collection
        try:
            TracerConfig.from_file("/nonexistent/config.json")
        except ConfigurationError as e:
            collector.add_error(e)
        
        # Test wrapper error collection
        ...
        with patch('pytracer.utils.log.get_logger') as mock_logger:
            mock_logger.return_value = MagicMock()
            reporter.report_errors(collector.get_errors())
