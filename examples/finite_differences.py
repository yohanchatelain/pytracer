...
def forward_diff_tiny_h(x):
    ...
    return np.mean((np.sin(x + h) - np.sin(x)) / h)


@pytracer.trace_function
def central_diff_optimal_h(x):
    h = EPS ** (1.0 / 3.0)
    return np.mean((np.sin(x + h) - np.sin(x - h)) / (2.0 * h))


def main():
    rng = np.random.default_rng()  # unseeded: per-run perturbation of x
    x = np.linspace(0.5, 1.5, 100) * (1.0 + rng.standard_normal() * 1e-8)
    exact = float(np.mean(np.cos(x)))

    fwd = float(forward_diff_tiny_h(x))
    ctr = float(central_diff_optimal_h(x))
